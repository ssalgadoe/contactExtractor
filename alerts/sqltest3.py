...
def isKeyInLineEx(keyWords,keyValue,line):
    total=0
    foundKeyList=""
    keyFound=False
    for i in range(len(keyWords)):
         if keyWords[i].strip() in line.lower():
             ...
             keyFound=True
    return keyFound,foundKeyList,total;
...
def myParser(urlRank,title,link,description,published,published2, keyWords, keyValue, outputFile,connection):
    ...
    keyFound,key,rank =  isKeyInLineEx(keyWords,keyValue,description)
    if keyFound: #key found in the line
        ...
        try:
            with connection.cursor() as cursor:
                ...
                cursor.execute(sql,(title, link, description, key,published,published2,rank))
                connection.commit()
        except:
