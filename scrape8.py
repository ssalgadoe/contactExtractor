...
for line in urlData:
    ...
    print(keyWords)
    print(keyWords[keyWordsLength-1])
    country = urlList[2]
    ...
    for li in lines:
        index=index+1
        nextLineUsed = False
        lastLineCounter = lastLineCounter + 1
        for name in keyWords:
            if name.lower() in li.lower(): #keyword is found in the line
                if firstLineFound == False: #first keyword is found. ready to record
                    firstLineFound = True
                if name.lower()==keyWords[keyWordsLength-1].lower():
                    lastLineFound = True
                    lastLineCounter = 0
                else:
                    lastLineFound = False
                    lastLineCounter = 0
                if len(name.strip())+3 > len(li.strip()): #keywords and values are in different row
                    ...
                    if not lines[index].lower() in keyWords: # key has a value
                        ...
                        countryFile.write('\n')
                        nextLineUsed = True #next line is validated
                        unknownLine=0
