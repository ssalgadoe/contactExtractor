...
def myParser(text,keyWords, countryFile):
    ...
    for index in range(0,lineLength-1):
        ...
        if keyFound: #key found in the line
            ...
            tempStr = lines[index]
            print(tempStr)
            java_escape = "eval(unescape"
            if java_escape in tempStr.lower().strip():
...
#if not ".cfg" in configFile:
   # print("Invalid Configuration file !!! e.g. python ",str(sys.argv[0]), " canada.cfg")
   # exit(0);   
print("config file->",configFile)
#read urllist from a file
tempStr=""
try:
    countryData = open(configFile, "r")
except:
...
for urlData in countryData:
    ...
    soup = bs4.BeautifulSoup(respData)
